  ...
  return (
    <article aria-labelledby={titleId} className='principle-card'>
      <div className='flex items-start justify-between'>
        <span className='text-sm text-foreground/70'>{t(principle.number)}</span>
        <Icon aria-hidden='true' className='size-5 text-primary' />
      </div>
      <h3 id={titleId} className='mt-14 font-heading text-2xl font-medium tracking-tight'>{t(principle.title)}</h3>
      <p className='mt-4 max-w-sm leading-7 text-foreground/75'>{t(principle.description)}</p>
    </article>
  )
}
